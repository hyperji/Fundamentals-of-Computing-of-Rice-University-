...
class Ship:
    ...
    def shoot(self):
        ...
        missiles.append(Sprite(ppos, vvv, 0, 0, missile_image, missile_info, missile_sound))
...
def draw(canvas):
    global time, my_ship, missiles, started
...
def rock_spawner():
    ...
    vel = [3*random.random()*random.choice([1,-1]), 2*random.random()*random.choice([1,-1])]
    ang = 0.5*random.random()
    ang_vel = 0.1*random.random()
